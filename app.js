...
function updateBrowserURL(contentType, contentID, replace = false) {
	// TODO and issue here, history state title changes randomly!
	const url = new URL(window.location.origin);
	url.searchParams.set(contentType, contentID);
	...
	window.history.pushState(
		{ tabTitle: pageTitleElement.innerText },
...
function showContent(value = null, type = null) {
	...
	getDownloadData((type = contentType), (id = contentID))
		...
		.finally((data) => {
			updateBrowserURL(contentType, contentID);
			return data;
		});
}
